...
async def getBalance(account, add,latestBlock):
    print(add)
    ...
    # z = requests.get(url.format('AAPL', api_key))
    # print(z)
...
# #############################################################################################################
def checkBalance(checkBlkNum, blockGap, iterations):
    ...
    with open("avg_balance.csv", 'a', newline='') as avg_balancefile:
        ...
        for add in addList:
            ...
            for balances in balanceList:
                # print(balances)
                balanceAdd += literal_eval(balances[add])
...
async def main():
    xInitializeBlk = 11995475     #167 days 12 hrs ago (Apr-04-2021 08:59:52 PM +UTC) contract created time
    latestblk = web3.eth.blockNumber
